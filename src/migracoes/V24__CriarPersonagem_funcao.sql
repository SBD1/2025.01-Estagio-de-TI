...
CREATE OR REPLACE FUNCTION adicionar_ataques_iniciais()
RETURNS TRIGGER AS $$
BEGIN
    -- Insere os ataques iniciais (mude os IDs conforme seus ataques existentes)
    INSERT INTO PersonagemAtaque (id_personagem, id_ataque)
    VALUES 
        (NEW.id_personagem, 1),
        (NEW.id_personagem, 2),
        (NEW.id_personagem, 3),
        ...
        (NEW.id_personagem, 8),
        (NEW.id_personagem, 9),
        (NEW.id_personagem, 10),
        (NEW.id_personagem, 11),
        (NEW.id_personagem, 12),
        (NEW.id_personagem, 13),
        (NEW.id_personagem, 14),
        (NEW.id_personagem, 15),
        (NEW.id_personagem, 16),
        (NEW.id_personagem, 17),
        (NEW.id_personagem, 18),
        (NEW.id_personagem, 19);
...
CREATE TRIGGER trigger_adicionar_ataques_iniciais
AFTER INSERT ON Estagiario
FOR EACH ROW
EXECUTE FUNCTION adicionar_ataques_iniciais();
